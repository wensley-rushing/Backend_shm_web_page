...
def Topology():
    ...
    Nodes=np.genfromtxt(Path.cwd()/'backend'/"Model"/"Nodes.txt",invalid_raise=False)
    # Member connections 
    conect = np.loadtxt(Path.cwd()/'backend'/"Model"/"Connc.txt",delimiter=',')
    # Frames asigns
    idele=np.genfromtxt(Path.cwd()/'backend'/"Model"/"Elementid.txt",invalid_raise=False)
    # Members Properties
    Members=np.genfromtxt(Path.cwd()/'backend'/"Model"/"Members.txt",invalid_raise=False)
    # Names
    Names = np.genfromtxt(Path.cwd()/'backend'/"Model"/"Nmes.txt",dtype='str',delimiter = '')
...
def Drawing2database(Nm):
    ...
    path = Path.cwd()/".."/"pickle_visualization"
    for i in range(Nm):
        idd  = str(Nm+1)
